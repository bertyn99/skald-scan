...
### A11. CRITICAL — Agent Category Fix (Discovered During Execution)

**ALL tasks originally assigned `unspecified-high` MUST use `deep` instead.**

**Root Cause**: The `unspecified-high` category model (`claude-opus-4-6`) does NOT execute tasks properly — it completes in ~30 seconds without creating files. This was confirmed during T2 execution (failed twice with `unspecified-high`, succeeded immediately with `deep`).

**Updated Agent Dispatch**:
| Wave | Tasks | Agents (UPDATED) |
|------|-------|------------------|
| 1 | 1 | T1 → `deep` ✅ (already completed) |
| 2 | 1 | T2 → `deep` ✅ (already completed) |
| 3 | 2 | T3 → `deep` ✅, T4 → `visual-engineering` ✅ |
| 4 | 2 | T5 → `deep`, T6 → `deep` |
| 5 | 4 | T7 → `deep`, T8 → `visual-engineering`, T9 → `visual-engineering`, T10 → `visual-engineering` |
| 6 | 4 | T11 → `deep`, T12 → `deep`, T13 → `visual-engineering`, T14 → `deep` |
| 7 | 2 | T15 → `deep`, T16 → `deep` |
| FINAL | 4 | F1 → `oracle`, F2 → `deep`, F3 → `deep`, F4 → `deep` |

**This overrides the Agent Dispatch Summary in the Execution Strategy section.**

### A12. nitro-test-utils Adoption (User Requested)

**MUST use `nitro-test-utils`** for all dashboard Nuxt server route tests going forward.

**Why**: User explicitly requested this. Benefits:
- `$fetchRaw` for testing Nitro API routes with real HTTP requests
- Cloudflare Workers support: `preset: 'cloudflare-module'` emulates bindings (D1, R2, KV) locally via `getPlatformProxy()`
- Session-aware testing via `createNitroSession()`
- Global or per-suite setup modes

**Requirements**:
- **Vitest v4+** required (currently dashboard uses v3.2.4, shared uses v3.2.4, reader uses v4.1.3)
- **Nitro v3** required (check dashboard's Nitro version)
- Install: `pnpm add -D nitro-test-utils` in dashboard
- Setup: Configure vitest with nitro-test-utils `setupTest` in vitest.config.ts
- For Cloudflare bindings emulation: `preset: 'cloudflare-module'` in test config

**External Reference**: `https://github.com/johannschopplich/nitro-test-utils`

**Action**: First agent working on T5 fix or T7+ MUST:
1. Upgrade dashboard Vitest from v3.2.4 → v4.x
2. Upgrade shared Vitest from v3.2.4 → v4.x (match reader's v4.1.3)
3. Install `nitro-test-utils` in dashboard
4. Configure vitest.config.ts with nitro-test-utils setup
5. Rewrite existing dashboard tests to use nitro-test-utils patterns

### A13. T5 Fix Directive (Critical — Blocking Wave 5)

**Current State**: T5 files exist but 7/11 tests fail with `TypeError: Cannot read properties of undefined (reading 'name')`

**Root Cause**: All 7 failing API routes use `${manga._.name}` / `${chapters._.name}` / `${pages._.name}` to build raw SQL. The Drizzle table objects from `@skald-scan/shared` don't resolve `._.name` in the Vitest test environment.

**Fix Strategy** (pick ONE):
1. **Preferred**: Use Drizzle query builder (`.from(manga).select()`) instead of raw SQL — this is the idiomatic Drizzle approach and avoids the `._.name` issue entirely
2. **Alternative**: Use literal table names (`'manga'`, `'chapters'`, `'pages'`) in raw SQL strings
3. **Also**: Adopt `nitro-test-utils` per A12 for proper route testing

**Files to fix** (all in `apps/dashboard/server/`):
- `api/manga/index.get.ts` — manga list
- `api/manga/index.post.ts` — create manga
- `api/manga/[mangaId].get.ts` — get manga
- `api/manga/[mangaId].put.ts` — update manga
- `api/manga/[mangaId]/chapters/index.get.ts` — list chapters
- `api/manga/[mangaId]/chapters/index.post.ts` — create chapter
- `api/manga/[mangaId]/chapters/[chapterId]/pages/[pageId].get.ts` — serve page
- `__tests__/storage.test.ts` — rewrite with nitro-test-utils patterns
- Plus: `vitest.config.ts` — upgrade to v4 + nitro-test-utils setup

**Agent executing T5 fix MUST**:
1. Upgrade dashboard Vitest → v4.x
2. Install + configure nitro-test-utils
3. Fix all 7 API routes to use Drizzle query builder (not raw SQL with `._.name`)
4. Rewrite storage.test.ts using nitro-test-utils `$fetchRaw`
5. Ensure ALL 11+ tests pass
6. Verify auth.test.ts still passes after Vitest upgrade
